...
export default class QuickLoanApp extends Component {
    constructor (props) {
        ...
        this.functions = {
            ...
            submitOnEnter: this.submitOnEnter.bind(this),
    ...
    removeItem(e,id) {
        ...
        let form = {...forms.single.children.items}
        Object.keys(form.children).map(key => {
            const child = form.children[key]
            if (child.children.id.value === id)
                delete(form.children[key])
        })
        forms.single.children.items = form
        this.setMyState(forms)
    }

    removePerson() {
        let forms = {...this.state.forms}
        let form = {...forms.single.children.person}
        form.value = ''
        forms.single.children.person = form
        this.setMyState(forms)
    }
    ...
    submitOnEnter(e,form)
    {
        if (e.key === 'Enter') {
            let element = document.getElementById('rapid_loaner_search')
            element.blur()
        }
    }
    ...
    render() {
        return (
            <section>
                ...
                <QuickLoanDisplay items={this.state.forms.single.children.items.children} functions={this.functions} person={this.state.forms.single.children.person} />
            </section>
        )
    }
}

QuickLoanApp.propTypes = {
