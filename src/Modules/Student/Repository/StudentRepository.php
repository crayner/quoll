...
class StudentRepository extends ServiceEntityRepository
{
    ...
    public function getStudentEnrolmentPaginationContent()
    {
        $result = $this->createQueryBuilder('s', 's.id')
            ->select(["CONCAT(".PersonNameManager::formatNameQuery('p', 'Student', 'Reversed').") AS student", "'' as rollOrder","'' AS rollGroup", "'' AS yearGroup", 's.id', 'p.id AS person_id',"'null' As enrolment",'p.status'])
            ->leftJoin('s.person', 'p')
            ->orderBy('p.surname', 'ASC')
            ->addOrderBy('p.preferredName', 'ASC')
